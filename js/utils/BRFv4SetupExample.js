	...
	example.init = function(type) {
		...
		if (!brfv4.sdkReady) {
			...
			if (brfv4.Rectangle && !resolution) {
				resolution = new brfv4.Rectangle(0, 0, 700, 600);
			}
			...
			if (type === 'picture') {
				// Start either using an image ...

				imageData.picture.setup(dom.getElement('_imageData'), imageData.onAvailable);
			} else {
				// ... or start using the webcam.
			...
			if (imageData.isAvailable()) {
				...
				resolution.setTo(0, 0, 1200, 700); // reset for webcam initialization
				imageData.init();
			}
		}
	};
