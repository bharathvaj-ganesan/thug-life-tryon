...
var brfv4Example = {
	appId: 'com.oic.examples', // Choose your own app id. 8 chars minimum.

	loader: { queuePreloader: null }, // preloading/example loading
	imageData: {
		// image data source handling
...
brfv4Example.start = function(id) {
	brfv4Example.loader.preload(
		[
			...
			'js/examples/sample.js' // start with this example
		],
		function() {
			...
			document.getElementById('selectBtn').addEventListener('click', function(e) {
				e.preventDefault();
				e.stopPropagation();
				html2canvas(document.querySelector('#_content')).then(canvas => {
					const imageDataURL = canvas.toDataURL('image/jpeg');
					const tmpLink = document.createElement('a');
					tmpLink.href = imageDataURL;
					tmpLink.download = 'ThugLife.jpg';
